...
async function xGet(pathQuery, { auth = 'app' } = {}) {
  ...
  return { status: r.status, ok: r.ok, json, text, limit: r.headers.get('x-rate-limit-remaining') };
}
...
function norm(t, author, kind) {
  const pm = t.public_metrics || {};
  return { id: String(t.id), created: t.created_at, text: cleanX(t.full_text || t.text), author: author || '', likes: pm.like_count || 0, rts: pm.retweet_count || 0, kind };
}
...
async function harvestActivity({ maxPosts = 300, maxLikes = 100, maxBookmarks = 100 } = {}) {
  ...
  const caps = { posts: 0, reposts: 0, likes: 0, bookmarks: 0 };
...
module.exports = {
  loadEnv, ENV, xGet, getUserId, getUserTweets, getLikedTweets, getBookmarks, harvestActivity,
